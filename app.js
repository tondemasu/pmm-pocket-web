...
const APP_VERSION = 'PMM Pocket Web v031';
...
function openEdit(type,idOrIndex){state.edit={type,id:null,index:null}; let obj={}; if(type==='self'){state.edit.id=idOrIndex; obj=state.data.members[idOrIndex]||{}} if(type==='other'){state.edit.index=Number(idOrIndex); obj=state.data.other_members[state.edit.index]||{}} if(type==='pending'){state.edit.index=Number(idOrIndex); obj=state.data.pending_self_members[state.edit.index]||{}} if(type==='other-new'){obj={id:uid(),name:''}} if(type==='pending-new'){obj={pending_id:uid(),name:'',member_type:'P',created_at:new Date().toISOString().slice(0,19)}}
  ...
  else html += `<div class="self-edit-tabs" role="tablist" aria-label="新メンバー編集切替"><button type="button" class="self-edit-tab active" data-self-edit-tab="activity">活動管理</button><button type="button" class="self-edit-tab" data-self-edit-tab="basic">基本情報</button></div><section class="self-edit-pane active" data-self-edit-pane="activity"><h4>活動管理</h4>${formInput('最終接触日','activity_last_contact',obj.activity_last_contact||obj.last_contact_date||'','date')}${formInput('約束したこと','promise',obj.promise||obj.activity_promise)}${formInput('次回アクション','next_action',obj.next_action)}${formDateWithSchedule('次回予定日','next_action_date',obj.next_action_date,'next_action')}${formInput('参加予定イベント','event',obj.event)}${formInput('温度感','temperature',obj.temperature)}${formText('メモ','activity_memo',obj.activity_memo||obj.memo)}${progressHtml(obj)}</section><section class="self-edit-pane" data-self-edit-pane="basic"><h4>基本情報</h4><div class="two-col">${formInput('名前','name',obj.name)}${formSelect('メンバー種類','member_type',obj.member_type,typeLabels)}${formInput('紹介者名','introducer_name',obj.introducer_name)}${formInput('地域','region',obj.region)}${formInput('タイトル','title',obj.title)}</div></section>`;
...
function showInfo(kind){const help=`<p><strong>推奨環境</strong></p><ul><li>iPhoneは <strong>Safari</strong> でお使いください</li><li>Androidは <strong>Chrome</strong> でお使いください</li><li>スマホではホーム画面に追加して使うのがおすすめです</li></ul><p class="notice strong-note">ブラウザのまま使う場合、下スワイプや再読み込みでスタート画面に戻ることがあります。保存前に更新すると未保存内容が失われる場合があるため、こまめに保存してください。</p><p>iPhoneのChromeでは、保存ファイルが「このiPhone内 → Chrome」に入りやすく、iCloud Driveへ直接保存しにくい場合があります。iCloud Driveで管理したい場合はSafariでの利用をおすすめします。</p><p><strong>基本の使い方</strong></p><ol><li>PC版PMMを閉じます</li><li>保存ファイル読込でPMM保存ファイルを読み込みます</li><li>現場で写真、メモ、要フォローを記録します</li><li>要フォローにチェックした人は「フォロー」画面でまとめて確認します</li><li>最後に保存ボタンで保存ファイルを作成します</li><li>PC版PMMで保存したファイルを開きます</li></ol><p><strong>PMM本体との役割分担</strong></p><ul><li>PMM本体：MAP管理、ポイント計算、作戦立案</li><li>Pocket Web：現場メモ、写真、要フォロー、共有データの受け渡し</li></ul><p><strong>自MAPメンバー表示について</strong></p><ul><li>自MAP一覧では P / PS / K / KS の種別表示は控えめにしています</li><li>フォロー対象として見る人は、種別ではなく「要フォロー」チェックで管理します</li><li>過去データの K（要フォロー）/ KS（要フォロー）は読み込み時に要フォロー扱いにします</li></ul><p>このWeb版はデータをサーバーへ保存しません。読み込んだJSONはブラウザ内で処理します。</p>`
 ...
 const downloads=`<p><strong>PC版PMMダウンロード</strong></p><p>PMM本体は、MAP管理・ポイント計算・印刷HTML出力を行うPC用アプリです。</p><div class="download-card"><a class="primary-btn" href="https://github.com/tondemasu/pmm-pocket-web/releases/download/pmm-v112/PMM_v112_Win.zip" target="_blank" rel="noopener">Windows版 PMM v112 をダウンロード</a><a class="primary-btn" href="https://github.com/tondemasu/pmm-pocket-web/releases/download/pmm-v112/PMM_v112_Mac.zip" target="_blank" rel="noopener">Mac版 PMM v112 をダウンロード</a></div><p class="notice">Macで初回起動できない場合は、PMM.appを右クリックまたはcontrolクリックして「開く」を選択してください。</p><p class="notice">同じJSONをPMM本体とPocket Webで同時に編集しないでください。</p>`;
